-- ========================================
-- Migration: 20251227095032_1d079ce4-40a4-444f-9b72-bf325c0add22.sql
-- ========================================
...
-- ========================================
-- Migration: 20251227095040_ab59e67b-587b-4e4d-98c2-d9dffee3dab2.sql
-- ========================================
...
-- ========================================
-- Migration: 20251227102824_f9d12fc0-8602-4b6b-bd4f-a85ff089b074.sql
-- ========================================
-- Add archetype column to store quiz results (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'archetype') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN archetype text;
  END IF;
END $$;

-- ========================================
-- Migration: 20251231005752_27f67131-4335-44bb-a28c-51f0e4f8569c.sql
-- ========================================
...
-- Add fluency_locked column to assessment_sessions to track lock state (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'fluency_locked') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN fluency_locked BOOLEAN NOT NULL DEFAULT false;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'fluency_locked_at') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN fluency_locked_at TIMESTAMP WITH TIME ZONE;
  END IF;
END $$;
...
-- ========================================
-- Migration: 20251231013254_9991f2a7-bd1d-4add-aaed-1651ccabf64e.sql
-- ========================================
...
-- ========================================
-- Migration: 20251231030934_50a34ff6-3db6-45c8-acb5-3831e9bd0c13.sql
-- ========================================
...
-- Add lock columns to assessment_sessions for new modules (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'confidence_locked') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN confidence_locked BOOLEAN NOT NULL DEFAULT false;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'confidence_locked_at') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN confidence_locked_at TIMESTAMP WITH TIME ZONE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'syntax_locked') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN syntax_locked BOOLEAN NOT NULL DEFAULT false;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'syntax_locked_at') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN syntax_locked_at TIMESTAMP WITH TIME ZONE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'conversation_locked') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN conversation_locked BOOLEAN NOT NULL DEFAULT false;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' AND table_name = 'assessment_sessions' AND column_name = 'conversation_locked_at') THEN
    ALTER TABLE public.assessment_sessions ADD COLUMN conversation_locked_at TIMESTAMP WITH TIME ZONE;
  END IF;
END $$;
...
-- ========================================
-- Migration: 20251231071040_1ebe9ac4-2e3d-4b62-b41d-9cd9c3d4290b.sql
-- ========================================
...
-- ========================================
-- Migration: 20251231092110_5971b3f9-2da0-443e-baa5-d6ae3f4fdf12.sql
-- ========================================
...
-- ========================================
-- Migration: 20251231104044_6fc3489f-fa02-4342-9be4-6dd512f5d20d.sql
-- ========================================
...
-- ========================================
-- Migration: 20251231111732_51d7876c-6308-4001-b2e2-94fa4d8b2105.sql
-- ========================================
...
-- ========================================
-- Migration: 20260101143952_f497cf23-2d76-4aa8-b5ba-2ed7c1ed110e.sql
-- ========================================
...
-- ========================================
-- Migration: 20260101144059_9a2a7c88-20dc-4933-9792-eb3d0301faad.sql
-- ========================================
...
-- ========================================
-- Migration: 20260101212344_sales_copilot.sql
-- ========================================
-- Sales Copilot Migration
-- Creates tables for leads, calls, and playbook management
-- NOTE: This file is now IDEMPOTENT (safe to run multiple times)
...
-- ========================================
-- Migration: 20260102164444_phrases_learning_ladder.sql
-- ========================================
...
-- Enums for phrase system (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'phrase_mode') THEN
    CREATE TYPE public.phrase_mode AS ENUM ('recall', 'recognition');
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'phrase_status') THEN
...
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scheduler_state') THEN
...
-- ========================================
-- Migration: 20260104181500_speaking_assessment_sessions.sql
-- ========================================
...
-- ========================================
-- Migration: 20260104182500_user_phoneme_stats.sql
-- ========================================
...
COMMENT ON COLUMN public.user_phoneme_stats.phoneme IS 'IPA symbol (e.g., /u/, /ÃƒÆ’Ã…Â Ãƒâ€šÃ‚Â/, /ÃƒÆ’Ã¢â‚¬Â°ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂºÃƒÆ’Ã…â€™Ãƒâ€ Ã¢â‚¬â„¢/)';
...
-- ========================================
-- Migration: 20260105084634_6166a086-77af-4461-b1d5-c93547eef292.sql
-- ========================================
...
-- ========================================
-- Migration: 20260105090245_47ba56fb-f334-49ec-aa5e-bb5f36837c10.sql
-- ========================================
...
-- ========================================
-- Migration: 20260105120000_confidence_phone_calls.sql
-- ========================================
...
-- ========================================
-- Migration: 20260105151240_comprehension_multi_select.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106_habits_goals.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106000000_scoring_traces.sql
-- ========================================
-- Scoring Traces Table
-- Stores unified scoring traces for calibration and debugging
...
-- ========================================
-- Migration: 20260106060141_7a945eae-97b0-41a8-b4d6-0de5ef408ec9.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106062554_c347c1f9-ce59-4c85-8c99-4283c038d912.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106122428_comprehension_items.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106122429_seed_comprehension_items.sql
-- ========================================
...
(
  ...
  14,
  5.6,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  ...
  '[
    ...
    {"id": "o4", "fr": "Marie a oubliÃƒÂ© son parapluie au travail.", "en": "Marie forgot her umbrella at work."},
    {"id": "o5", "fr": "Marie cherche ses clÃƒÂ©s.", "en": "Marie is looking for her keys."},
    {"id": "o6", "fr": "Il fait trÃƒÂ¨s beau aujourd''hui.", "en": "The weather is very sunny today."},
    {"id": "o7", "fr": "Le parapluie est cassÃƒÂ©.", "en": "The umbrella is broken."},
    {"id": "o8", "fr": "Marie va ÃƒÂ  la plage.", "en": "Marie is going to the beach."}
  ]'::jsonb,
...
(
  ...
  'fr-FR',
  'A1',
  'Au cafÃƒÂ©, Paul commande un thÃƒÂ© sans sucre, attend deux minutes, puis demande l''addition et son ticket avant de partir.',
  20,
  8.0,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Paul commande un thÃƒÂ© sans sucre.", "en": "Paul orders a tea with no sugar."},
    ...
    {"id": "o4", "fr": "Paul commande un cafÃƒÂ© au lait.", "en": "Paul orders a coffee with milk."},
    {"id": "o5", "fr": "Il demande la carte des desserts.", "en": "He asks for the dessert menu."},
    {"id": "o6", "fr": "Il reste au cafÃƒÂ© pendant une heure.", "en": "He stays at the cafe for an hour."},
...
(
  ...
  'fr-FR',
  'A2',
  'Dans le bus, quelqu''un a oubliÃƒÂ© un sac bleu sous un siÃƒÂ¨ge. Le chauffeur l''annonce au micro, le met devant lui, et dit de le rÃƒÂ©cupÃƒÂ©rer au terminus.',
  28,
  11.2,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Quelqu''un a oubliÃƒÂ© un sac bleu.", "en": "Someone forgot a blue bag."},
    ...
    {"id": "o4", "fr": "On peut rÃƒÂ©cupÃƒÂ©rer le sac au terminus.", "en": "You can pick up the bag at the end of the line."},
    {"id": "o5", "fr": "Le sac est rouge.", "en": "The bag is red."},
    {"id": "o6", "fr": "Le chauffeur jette le sac.", "en": "The driver throws the bag away."},
    {"id": "o7", "fr": "Il faut aller au commissariat pour le rÃƒÂ©cupÃƒÂ©rer.", "en": "You must go to the police station to retrieve it."},
    {"id": "o8", "fr": "Le bus s''arrÃƒÂªte tout de suite pour chercher le propriÃƒÂ©taire.", "en": "The bus stops immediately to find the owner."}
  ]'::jsonb,
...
(
  ...
  'fr-FR',
  'A2',
  'On se retrouve ÃƒÂ  la station RÃƒÂ©publique ÃƒÂ  18 h, prÃƒÂ¨s de la sortie 3. DÃƒÂ©solÃƒÂ©, mon bus est bloquÃƒÂ© dans les embouteillages, je serai dix minutes en retard. Ne pars pas.',
  32,
  12.8,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Le rendez-vous est ÃƒÂ  la station RÃƒÂ©publique.", "en": "The meeting point is RÃƒÂ©publique station."},
    {"id": "o2", "fr": "Le rendez-vous est ÃƒÂ  18 h.", "en": "The meeting is at 6 pm."},
    {"id": "o3", "fr": "La personne aura environ dix minutes de retard.", "en": "The person will be about ten minutes late."},
    {"id": "o4", "fr": "Son bus est bloquÃƒÂ© dans les embouteillages.", "en": "Their bus is stuck in traffic."},
    {"id": "o5", "fr": "Le rendez-vous est ÃƒÂ  8 h.", "en": "The meeting is at 8 am."},
    {"id": "o6", "fr": "Ils se retrouvent ÃƒÂ  la station Bastille.", "en": "They are meeting at Bastille station."},
    {"id": "o7", "fr": "La personne est dÃƒÂ©jÃƒÂ  arrivÃƒÂ©e.", "en": "The person has already arrived."},
...
(
  ...
  'fr-FR',
  'A2',
  'Une voisine crie dans la rue : son chat est coincÃƒÂ© dans un arbre depuis une heure. Elle veut appeler les pompiers, mais elle ne connaÃƒÂ®t pas le numÃƒÂ©ro et son tÃƒÂ©lÃƒÂ©phone est presque dÃƒÂ©chargÃƒÂ©. Elle demande ÃƒÂ  un passant de l''aider.',
  42,
  16.8,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Un chat est coincÃƒÂ© dans un arbre.", "en": "A cat is stuck in a tree."},
    {"id": "o2", "fr": "Elle veut appeler les pompiers.", "en": "She wants to call the fire department."},
    {"id": "o3", "fr": "Elle ne connaÃƒÂ®t pas le numÃƒÂ©ro ÃƒÂ  appeler.", "en": "She doesn''t know the number to call."},
    {"id": "o4", "fr": "Son tÃƒÂ©lÃƒÂ©phone est presque dÃƒÂ©chargÃƒÂ©.", "en": "Her phone is almost out of battery."},
    {"id": "o5", "fr": "Un chien est coincÃƒÂ© dans un arbre.", "en": "A dog is stuck in a tree."},
    {"id": "o6", "fr": "Les pompiers sont dÃƒÂ©jÃƒÂ  en route.", "en": "The fire department is already on the way."},
...
(
  ...
  'fr-FR',
  'A2',
  'On dÃƒÂ©cale la rÃƒÂ©union de cet aprÃƒÂ¨s-midi ÃƒÂ  demain matin, ÃƒÂ  9 h, parce que le client est malade. J''envoie tout de suite un e-mail avec la nouvelle heure et le lien visio. Garde ton aprÃƒÂ¨s-midi libre.',
  37,
  14.8,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "La rÃƒÂ©union est dÃƒÂ©placÃƒÂ©e ÃƒÂ  demain matin, ÃƒÂ  9 h.", "en": "The meeting is moved to tomorrow morning at 9."},
    ...
    {"id": "o4", "fr": "Le lien visio est envoyÃƒÂ© par e-mail.", "en": "The video-call link is sent by email."},
    {"id": "o5", "fr": "La rÃƒÂ©union reste cet aprÃƒÂ¨s-midi.", "en": "The meeting stays this afternoon."},
    {"id": "o6", "fr": "La rÃƒÂ©union est annulÃƒÂ©e dÃƒÂ©finitivement.", "en": "The meeting is canceled forever."},
    {"id": "o7", "fr": "Le client est en vacances.", "en": "The client is on vacation."},
    {"id": "o8", "fr": "La rÃƒÂ©union est dÃƒÂ©placÃƒÂ©e ÃƒÂ  ce soir.", "en": "The meeting is moved to tonight."}
  ]'::jsonb,
  '{"correct_option_ids": ["o1", "o2", "o3", "o4"]}'::jsonb
),
(
  ...
  'fr-FR',
  'B1',
  'Annonce en gare : le train pour Lyon de 17 h 12 est annulÃƒÂ© ÃƒÂ  cause d''un problÃƒÂ¨me technique. Un bus de remplacement part du quai 5 dans vingt minutes. Pour un remboursement, allez au guichet avec votre billet. Les autres trains restent ÃƒÂ  l''heure.',
  45,
  18.0,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Le train pour Lyon de 17 h 12 est annulÃƒÂ©.", "en": "The 5:12 pm train to Lyon is canceled."},
    {"id": "o2", "fr": "C''est ÃƒÂ  cause d''un problÃƒÂ¨me technique.", "en": "It is due to a technical problem."},
    ...
    {"id": "o8", "fr": "Tous les trains sont annulÃƒÂ©s aujourd''hui.", "en": "All trains are canceled today."}
  ]'::jsonb,
  '{"correct_option_ids": ["o1", "o2", "o3", "o4"]}'::jsonb
),
(
  ...
  'fr-FR',
  'B1',
  'Dans la colocation, ils se disputent la facture d''ÃƒÂ©lectricitÃƒÂ© : l''un dit qu''il ne cuisine jamais, l''autre laisse la lumiÃƒÂ¨re allumÃƒÂ©e. AprÃƒÂ¨s quelques minutes, ils se calment et dÃƒÂ©cident de suivre leur consommation avec une appli pendant un mois, ÃƒÂ  partir d''aujourd''hui, puis de partager la facture selon l''usage rÃƒÂ©el.',
  50,
  20.0,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Ils se disputent la facture d''ÃƒÂ©lectricitÃƒÂ©.", "en": "They argue about the electricity bill."},
    {"id": "o2", "fr": "Ils dÃƒÂ©cident de suivre leur consommation avec une appli.", "en": "They decide to track their usage with an app."},
    {"id": "o3", "fr": "Ils le font pendant un mois.", "en": "They do it for one month."},
    {"id": "o4", "fr": "Ils partageront la facture selon l''usage rÃƒÂ©el.", "en": "They will split the bill based on actual usage."},
    {"id": "o5", "fr": "Ils se disputent la facture d''eau.", "en": "They argue about the water bill."},
    {"id": "o6", "fr": "Ils dÃƒÂ©cident de ne plus payer la facture.", "en": "They decide to stop paying the bill."},
    {"id": "o7", "fr": "Ils partagent forcÃƒÂ©ment 50/50.", "en": "They will definitely split it 50/50."},
    {"id": "o8", "fr": "Ils achÃƒÂ¨tent un nouveau frigo.", "en": "They buy a new fridge."}
  ]'::jsonb,
  '{"correct_option_ids": ["o1", "o2", "o3", "o4"]}'::jsonb
),
(
  ...
  'fr-FR',
  'A2',
  'Au restaurant, elle prÃƒÂ©cise qu''elle est allergique aux noix. Le serveur part vÃƒÂ©rifier en cuisine si la sauce contient des amandes. Il revient : il y en a. Elle change de plat et prend une salade sans sauce.',
  38,
  15.2,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    ...
    {"id": "o2", "fr": "Le serveur vÃƒÂ©rifie la sauce en cuisine.", "en": "The waiter checks the sauce in the kitchen."},
    ...
    {"id": "o7", "fr": "Elle garde le mÃƒÂªme plat.", "en": "She keeps the same dish."},
...
(
  ...
  'fr-FR',
  'B1',
  'Le recruteur propose un entretien lundi ÃƒÂ  11 h pour un poste de chef de projet. Le candidat demande si c''est 100 % ÃƒÂ  distance ; on lui rÃƒÂ©pond : hybride, trois jours au bureau. Il demande la fourchette de salaire. Ils fixent un second appel mercredi avec la RH.',
  50,
  20.0,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Un entretien est proposÃƒÂ© lundi ÃƒÂ  11 h.", "en": "An interview is proposed for Monday at 11."},
    {"id": "o2", "fr": "On lui rÃƒÂ©pond : hybride, trois jours au bureau.", "en": "They answer: hybrid, three days in the office."},
    ...
    {"id": "o5", "fr": "On lui rÃƒÂ©pond : c''est totalement ÃƒÂ  distance.", "en": "They answer: it is fully remote."},
    {"id": "o6", "fr": "L''entretien est prÃƒÂ©vu dimanche matin.", "en": "The interview is set for Sunday morning."},
...
(
  ...
  'fr-FR',
  'B2',
  'Au magasin, LÃƒÂ©a revient avec un casque audio qui grÃƒÂ©sille. Elle veut ÃƒÂªtre remboursÃƒÂ©e, mais elle a perdu le ticket de caisse. Le vendeur propose un ÃƒÂ©change ou un avoir. Elle insiste pour un remboursement sur sa carte, alors il appelle la responsable.',
  43,
  17.2,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    {"id": "o1", "fr": "Le casque audio grÃƒÂ©sille.", "en": "The headphones crackle."},
    {"id": "o2", "fr": "LÃƒÂ©a a perdu le ticket de caisse.", "en": "LÃƒÂ©a lost the receipt."},
    {"id": "o3", "fr": "Le vendeur propose un ÃƒÂ©change ou un avoir.", "en": "The seller offers an exchange or store credit."},
    {"id": "o4", "fr": "Le vendeur appelle la responsable.", "en": "The seller calls the manager."},
    {"id": "o5", "fr": "LÃƒÂ©a a le ticket de caisse.", "en": "LÃƒÂ©a has the receipt."},
...
(
  ...
  'fr-FR',
  'B2',
  'Lors d''une rÃƒÂ©union de quartier, on propose de fermer la rue aux voitures le week-end. Certains commerÃƒÂ§ants sont pour, d''autres craignent de perdre des clients. La mairie propose un essai d''un mois et un vote ensuite. Un habitant rappelle qu''il faut garder un accÃƒÂ¨s pour les ambulances.',
  47,
  18.8,
  'Que se passe-t-il ? SÃƒÂ©lectionne toutes les affirmations vraies. Il peut y en avoir plusieurs.',
  'What is going on? Select all statements that are true. There may be more than one.',
  '[
    ...
    {"id": "o2", "fr": "Certains commerÃƒÂ§ants sont pour, d''autres sont inquiets.", "en": "Some shop owners support it; others are worried."},
    {"id": "o3", "fr": "La mairie propose un essai d''un mois, puis un vote.", "en": "City hall proposes a one-month trial, then a vote."},
    {"id": "o4", "fr": "Il faut garder un accÃƒÂ¨s pour les ambulances.", "en": "They must keep access for ambulances."},
    {"id": "o5", "fr": "La rue sera fermÃƒÂ©e tous les jours, toute l''annÃƒÂ©e.", "en": "The street will be closed every day all year."},
    {"id": "o6", "fr": "Tout le monde est d''accord immÃƒÂ©diatement.", "en": "Everyone agrees immediately."},
    {"id": "o7", "fr": "La mairie abandonne l''idÃƒÂ©e dÃƒÂ¨s maintenant.", "en": "City hall drops the idea right away."},
...
-- ========================================
-- Migration: 20260106130000_create_phrases_audio_bucket.sql
-- ========================================
...
-- To create manually:
-- 1. Go to Supabase Dashboard ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Storage
-- 2. Click "New bucket"
...
-- ========================================
-- Migration: 20260106130001_fix_comprehension_audio_policies.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106140000_phrases_insert_policy.sql
-- ========================================
...
-- ========================================
-- Migration: 20260106140001_update_comprehension_audio_urls.sql
-- ========================================
...
-- ========================================
-- Migration: 20260107000000_unified_exam.sql
-- ========================================
...
-- ========================================
-- SUPPLEMENTAL: STORAGE BUCKETS (ensure both exist)
-- ========================================

-- Create comprehension-audio bucket if not exists
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'comprehension-audio',
  'comprehension-audio',
...
-- Ensure phrases-audio bucket exists (may already exist from migration)
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'phrases-audio',
  'phrases-audio',
...
-- Storage policies for comprehension-audio
DROP POLICY IF EXISTS "Public read access for comprehension-audio" ON storage.objects;
CREATE POLICY "Public read access for comprehension-audio"
ON storage.objects FOR SELECT USING (bucket_id = 'comprehension-audio');

DROP POLICY IF EXISTS "Authenticated users can upload to comprehension-audio" ON storage.objects;
CREATE POLICY "Authenticated users can upload to comprehension-audio"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'comprehension-audio' AND auth.role() = 'authenticated');

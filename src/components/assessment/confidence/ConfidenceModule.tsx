...
export function ConfidenceModule({ sessionId, onComplete, onSkip }: ConfidenceModuleProps) {
  ...
  const handleSkip = async () => {
    // If in questionnaire phase, save partial answers first
    if (phase === 'questionnaire' && savePartialRef.current) {
      console.log('[Confidence] Saving partial questionnaire on skip');
      try {
        await savePartialRef.current();
        console.log('[Confidence] Partial save completed');
      } catch (error) {
        console.error('[Confidence] Partial save failed:', error);
      }
    }
    // Then call the parent's onSkip
    onSkip?.();
  };
